...
// componentTemplate is for boilerplate code for components.
...
{{- else}}
    if asserted, ok := ecs.GetComponent(e, {{$name}}CID).(*{{$name}}); ok {
        return asserted
    }
	return nil
{{- end}}
...
func main() {
	...
	tmpl, err := template.New("boilerplate").Parse(componentTemplate)
	if err != nil {
