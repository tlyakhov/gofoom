...
func (g *Grid) fieldVector4(index int, field *state.PropertyGridField) {
	origValue := ""
	for i, v := range field.Values {
		if i != 0 {
			origValue += ", "
		}
		vec := v.Elem().Interface().(concepts.Vector4)
	...
	box.Connect("activate", func(_ *gtk.Entry) {
		...
		vec, err := concepts.ParseVector4(text)
		if err != nil {
			log.Printf("Couldn't parse Vector4 from user entry. %v\n", err)
			box.SetText(origValue)
			g.Container.GrabFocus()
			return
		}
