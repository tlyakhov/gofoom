...
func (mw *MapWidget) DrawBodyAngle(e *core.Body) {
	...
	mw.Context.LineTo(e.Pos.Now[0]+math.Cos(astart)*e.BoundingRadius*2, e.Pos.Now[1]+math.Sin(astart)*e.BoundingRadius*2)
	mw.Context.DrawArc(e.Pos.Now[0], e.Pos.Now[1], e.BoundingRadius*2, astart, aend)
	mw.Context.MoveTo(e.Pos.Now[0], e.Pos.Now[1])
	mw.Context.LineTo(e.Pos.Now[0]+math.Cos(aend)*e.BoundingRadius*2, e.Pos.Now[1]+math.Sin(aend)*e.BoundingRadius*2)
...
func (mw *MapWidget) DrawBody(ibody *concepts.EntityRef) {
	...
	if player := behaviors.PlayerFromDb(ibody); player != nil {
		...
		mw.Context.NewSubPath()
		mw.Context.DrawArc(body.Pos.Now[0], body.Pos.Now[1], body.BoundingRadius/2, 0, math.Pi*2)
		mw.Context.ClosePath()
		mw.Context.Stroke()
	...
	mw.Context.SetLineWidth(1)
	mw.Context.NewSubPath()
	mw.Context.DrawArc(body.Pos.Now[0], body.Pos.Now[1], body.BoundingRadius, 0, math.Pi*2)
	mw.Context.ClosePath()
	mw.Context.Stroke()
