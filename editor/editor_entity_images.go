...
func (e *Editor) materialSelectionBorderColor(entity ecs.Entity) *concepts.Vector4 {
	if materials.GetShader(entity) != nil {
...
func (e *Editor) imageForSector(entity ecs.Entity) image.Image {
	...
	for _, segment := range sector.Segments {
		...
		y := (segment.P.Render[1] - sector.Min[1]) * float64(h) * dy / (sector.Max[1] - sector.Min[1])
		context.MoveTo(x, y)
		x = (segment.Next.P.Render[0] - sector.Min[0]) * float64(w) * dx / (sector.Max[0] - sector.Min[0])
		y = (segment.Next.P.Render[1] - sector.Min[1]) * float64(h) * dy / (sector.Max[1] - sector.Min[1])
		context.LineTo(x, y)
		context.ClosePath()
		context.Stroke()
	}
