...
func (wc *WorldController) Always() {
	...
	for entity, c := range wc.DB.Components[core.BodyComponentIndex] {
		body := c.(*core.Body)
		if !body.Active || body.SectorEntityRef.Nil() {
			continue
		}
		for _, pvs := range body.Sector().PVSBody {
			set.Act(wc.DB.EntityRef(entity), pvs.Ref(), concepts.ControllerProximity)
		}
	}
}
			...
			for _, segment := range sector.Segments {
				for _, segment2 := range sector2.Segments {
					if segment.Matches(segment2) {
						segment2.AdjacentSector = sector.Ref()
						segment2.AdjacentSegment = segment
						segment.AdjacentSector = sector2.Ref()
...
func CreateTestWorld(db *concepts.EntityComponentDB) {
	...
	imaterial := archetypes.CreateBasicMaterial(db, true)
	nmat := concepts.NamedFromDb(imaterial)
	nmat.Name = "Default Material"
	//tex.Diffuse = color.NRGBA{R: 128, G: 100, B: 50, A: 255}
	tex := materials.ImageFromDb(imaterial)
	tex.Source = "data/grass.jpg"
	tex.Filter = false
	tex.GenerateMipMaps = true
	tex.Load()
	tiled := materials.TiledFromDb(imaterial)
	tiled.Scale = 5.0
...
func CreateTestWorld2(db *concepts.EntityComponentDB) {
	...
	sector3 := CreateTestSector(db, "sector3", 200, -100, 200)
