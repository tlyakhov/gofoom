...
func WallHiPick(s *state.SlicePortal) {
	if s.Y >= s.ClippedStart && s.Y < s.AdjClippedTop {
		s.PickedElements = append(s.PickedElements, state.PickedElement{Type: "hi", Attachable: s.Segment})
	}
}
...
func WallLowPick(s *state.SlicePortal) {
	if s.Y >= s.AdjClippedBottom && s.Y < s.ClippedEnd {
		s.PickedElements = append(s.PickedElements, state.PickedElement{Type: "lo", Attachable: s.Segment})
	}
}
